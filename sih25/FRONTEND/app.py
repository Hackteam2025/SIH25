    ...
    html.Details([
        ...
        html.Div([
            dcc.Upload(
                id="file-upload",
                children=html.Div([
                    "🗂️ Drag and Drop NetCDF Files or ",
                    html.A("Click to Select", style={"color": "#667eea", "font-weight": "bold"})
                ]),
                style={
                    "width": "100%",
                    "height": "80px",
            ...
            html.Div(id="processing-status-display", style={"margin-top": "15px"})
...
# File processing - connects to existing DataOps orchestrator
@app.callback(
...
def handle_file_processing(process_clicks, file_contents, filenames):
    if process_clicks > 0 and file_contents:
        try:
            # Process files using existing DataOps orchestrator
            response = requests.post(
            ...
            if response.status_code == 200:
                return html.Div([
                    "✅ Files processed successfully! Data is now available for analysis.",
                ], style={"color": "#10b981", "font-weight": "bold"})
            else:
                return html.Div([
                    "⚠️ Processing failed. Please check file formats and try again.",
    ...
    return "Ready to process NetCDF files"
